...
public:
    ...
    static CANPacket create_switch_control_mode_command(const Motor& motor, Control_Mode_Code mode);
    static CANPacket create_mit_control_command(const Motor& motor, const MITParam& mit_param);
    static CANPacket create_pos_vel_control_command(const Motor& motor, float pos, float vel);
    static CANPacket create_vel_control_command(const Motor& motor, float vel);
...
private:
    static std::vector<uint8_t> pack_switch_control_mode_data(const Motor& motor, Control_Mode_Code mode);
    static std::vector<uint8_t> pack_mit_control_data(MotorType motor_type,
                                                      const MITParam& mit_param);
    static std::vector<uint8_t> pack_pos_vel_control_data(float pos, float vel);
    static std::vector<uint8_t> pack_vel_control_data(float vel);
