...
void DMDeviceCollection::switch_control_mode_one(int i, Control_Mode_Code mode)
{
    CANPacket switch_mode_cmd = CanPacketEncoder::create_switch_control_mode_command(get_dm_devices()[i]->get_motor(), mode);
    send_command_to_device(get_dm_devices()[i], switch_mode_cmd);
}

void DMDeviceCollection::switch_control_mode_all(Control_Mode_Code mode)
{
    for (auto dm_device : get_dm_devices()) {
        CANPacket switch_mode_cmd = CanPacketEncoder::create_switch_control_mode_command(dm_device->get_motor(), mode);
        send_command_to_device(dm_device, switch_mode_cmd);
    }
}
...
void DMDeviceCollection::pos_vel_control_one(int i, float pos, float vel)
{
    CANPacket pos_vel_cmd =
        CanPacketEncoder::create_pos_vel_control_command(get_dm_devices()[i]->get_motor(), pos, vel);
    send_command_to_device(get_dm_devices()[i], pos_vel_cmd);
}

void DMDeviceCollection::pos_vel_control_all(const std::vector<std::pair<float, float>> &pos_vels)
{
    std::cout << "control_all pos_vels size: " << pos_vels.size() << std::endl;
    for (size_t i = 0; i < pos_vels.size(); i++) {
        pos_vel_control_one(i, pos_vels[i].first, pos_vels[i].second);
    }
}

void DMDeviceCollection::vel_control_one(int i, float vel)
{
    CANPacket vel_cmd =
        CanPacketEncoder::create_vel_control_command(get_dm_devices()[i]->get_motor(), vel);
    send_command_to_device(get_dm_devices()[i], vel_cmd);
}

void DMDeviceCollection::vel_control_all(const std::vector<float> &vels)
{
    // std::cout << "control_all pos_vels size: " << pos_vels.size() << std::endl;
    for (size_t i = 0; i < vels.size(); i++) {
        vel_control_one(i, vels[i]);
    }
}
