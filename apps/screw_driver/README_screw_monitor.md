...
```bash
cd D:/SCREW06181656/screw_driver
cmake -S . -B build -DUSE_OPENARM_CAN=ON
cmake --build build
```

If the project is on Linux, use the Linux path instead of `D:/...`.

Run with the default CAN and motor IDs:

```bash
./build/screw_monitor --openarm --can can0 --send-id 0x06 --recv-id 0x16 --motor-type 1
```

Run for 10 seconds, sampling every 20 ms:

```bash
./build/screw_monitor --openarm --duration-sec 10 --period-ms 20
```

Command a slow velocity while monitoring feedback:

```bash
./build/screw_monitor --openarm --speed-rpm 60 --duration-sec 5
```
