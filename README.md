...
python apps/screw_demo/main.py --config apps/screw_demo/config/default.json
```
...
`config/default.json` 当前默认把力日志写入 `artifacts/logs/screw_demo`；真实实验仍建议显式确认该目录，避免误把实验产物写入源码目录。
...
```bash
python apps/screw_demo/main.py --help
python lerobot_robot_screw/scripts/recording/record_main_insert_dataset.py --help
python lerobot_robot_screw/scripts/recording/record_camera_insert_dataset_parallel.py --help
```
...
Socket 服务启动后，先用 `screw_client.py` 或对应测试脚本做低速、短时、无负载测试。电批服务必须具备通信超时、断开后的零速/保持、故障和急停处理，不能只依赖上位机正常退出。
